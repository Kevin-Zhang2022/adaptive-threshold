...
def train_model(model, train_loader, val_loader, test_loader,
                ...
                num_epochs=10, initial_lr=0.1, step_size=5, gamma=0.1, test_train=False, **kwargs):
    criterion = nn.CrossEntropyLoss()
...
def train(datasets_paths, trials, path2tab, path2trained,
            ...
            for model_name, model in models.items():
                ...
                best_model_state, best_accuracies = train_model(model, train_loader, val_loader, test_loader,
                                                               ...
                                                               initial_lr = initial_lr, step_size=step_size, gamma=0.1,
                                                                test_train=test_train, save_epochs=kwargs['save_epochs'])
    ...
    # model_save_path = os.path.join(path2trained, "temp.pth")
    # torch.save(temp, model_save_path)  # Save the entire temp dictionary
    # print(f"Best models dictionary saved to {model_save_path}")
    #
    # for (model_name, path), data in temp.items():
...
def test_normal(dataset, trials, batch_size=32, method='agss'):
    ...
    if method == 'at':
        state_dict = torch.load(f'bestmodel/shufflenet_{method}_{dataset_name}.pth')
        state_dict.pop('a.sleaky.threshold', None)
    ...
    return np.mean(average_testacc)
