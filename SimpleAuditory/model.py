...
class CT(nn.Module):
    ...
    def forward(self, inp,  **kwargs):
        ...
        # w =
        mem_rec = []
        T = ((self.range[0]+self.range[1])/2+self.t*0.8).clamp(self.range[0], self.range[1])
        ...
        if 'w_rec' in kwargs:
            return out.squeeze(0).squeeze(0).detach().numpy(), T.repeat(224).detach().numpy()
        else:
            return out
...
class AT(nn.Module):
    ...
    def forward(self, inp,  **kwargs):
        ...
        for t in range(inp.shape[-1]):
            ...
            temp = (self.range[0]+self.range[1])/2+torch.matmul(self.w, x.unsqueeze(-1)).squeeze(-1)
            T = temp.clamp(self.range[0], self.range[1]).unsqueeze(-1).repeat((1, h))
            self.sleaky.threshold = T
            t_rec.append(T)
            # I = x-T+1
            spk, mem = self.sleaky(x, mem)
            out[:, 0, :, t] = spk.squeeze(-1)
        if 'w_rec' in kwargs:
            return out.squeeze(0).squeeze(0).detach().numpy(), torch.stack(t_rec).squeeze(1)[:,0].detach().numpy()
        else:
            return out
